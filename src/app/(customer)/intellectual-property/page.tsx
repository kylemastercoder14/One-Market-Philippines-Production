...
const IntellectualProperty = async () => {
  const intellectual = await db.adminPolicies.findMany({});
  return (
    <div className='pb-10'>
      ...
      <h1 className="text-center mt-10 text-2xl font-bold">
        1 Market Philippines | Intellectual Property Policy
      </h1>
      <p className="text-center mt-3 text-muted-foreground">
        Effective Date:{" "}
        {format(new Date(intellectual[0].updatedAt), "MMMM dd, yyyy")}
      </p>
      <div
        className="px-10 md:px-[200px] mt-8 space-y-3 text-gray-700 leading-loose"
        dangerouslySetInnerHTML={{ __html: intellectual[0].intellectualProperty || "" }}
      />
    </div>
  );
};
